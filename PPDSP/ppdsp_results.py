...
def main():
    ...
    for filename in os.listdir(log_dir):
        if filename.endswith(".out") and filename.startswith("ppdsp_"):
    ...
    # Dynamic BKB Calculation for Maximization (PPDSP)
    for (inst, req), group in df.groupby(['Instance', 'Requests']):
        optimal_runs = group[(group['Time(s)'] < 3590) & (group['Objective(NetProfit)'].notna())]
        ...
        if not optimal_runs.empty:
            ...
            if not valid_bounds.empty:
                bkb = valid_bounds.min() # Min Upper Bound
            else:
                bkb = np.nan
    ...
    cols = ['Instance', 'Requests', 'Method', 'Objective(NetProfit)', 'BestBound', 'BKB', 'Gap(%)', 'Time(s)']
    df = df[cols]
