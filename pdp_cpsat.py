...
class PDP_CPSAT(PDP_reform):
    def __init__(self, pdpName, num_reqs, num_vehs, capacity, knn, increment=None):
        ...
        self.insName = f"pdp_{pdpName}_r{num_reqs}v{num_vehs}k{knn}_cpsat"
    ...
    def genCpModel(self):
        ...
        for r in range(num_reqs):
            self.model.AddExactlyOne([self.y[r, t] for t in range(num_vehs)])

        # Eq.3 & Eq.4 mapping: Node Activation and Net Demand (Delta) Logic
        for t in range(num_vehs):
            ...
            for i in range(num_nodes):
                if i != DEPOT:
                    ...
                    reqs_pickup = [r for r in range(num_reqs) if self.requestList[r][1] == i]
                    reqs_delivery = [r for r in range(num_reqs) if self.requestList[r][2] == i]
                    ...
                    pickup_sum = sum(self.requestList[r][0] * self.y[r, t] for r in reqs_pickup)
                    delivery_sum = sum(self.requestList[r][0] * self.y[r, t] for r in reqs_delivery)
        ...
        for t in range(num_vehs):
            ...
            for r in range(num_reqs):
                p_node = self.requestList[r][1]
                d_node = self.requestList[r][2]
                if p_node != d_node:
